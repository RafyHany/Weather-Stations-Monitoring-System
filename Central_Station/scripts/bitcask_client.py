...
BASE_URL = "http://127.0.0.1:30080/bitcask"
...
def view_all(filename=None):
    try:
        ...
        data = response.json()
        ts = int(time.time())
        filename = filename or f"./Central_Station/scripts/data/{ts}.csv"
...
def perf_worker(thread_id):
    ts = int(time.time())
    filename = f"./Central_Station/scripts/threads/{ts}_thread_{thread_id}.csv"
